...
def demo(data):
    ...
    # tic = time.time()
    # Z3, E3 = solve_lrr(X, A, lamb, reg=1, alm_type=0)
    # obj1 = np.sum(np.linalg.svd(Z3)[1]) + lamb * np.sum(np.sqrt(np.sum(E3 ** 2,
    #                                                                    1)))
    # print("Elapsed time:", time.time() - tic)
    #
    # print("objective value=", obj1)
    ...
    # tic = time.time()
    # Z4, E4 = solve_lrr(X, A, lamb, reg=1, alm_type=1)
    # obj2 = np.sum(np.linalg.svd(Z4)[1]) + lamb * np.sum(np.sqrt(np.sum(E4 ** 2,
    #                                                                    1)))
    # print("Elapsed time:", time.time() - tic)
    # print("objective value=", obj2)
    #
    # diff = np.max(np.abs(Z3 - Z4))
    #
    # print("### Warning: difference of the solution found by those two\
    #       approaches: |Z3 - Z4|_inf=", diff)
    return [Z2, E2]
